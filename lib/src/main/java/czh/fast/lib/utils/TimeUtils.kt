...
object TimeUtils {
    ...
    val LEVEL_MINUTE = 4
    val LEVEL_SECOND = 5
    val LEVELS = intArrayOf(LEVEL_YEAR, LEVEL_MONTH, LEVEL_DAY, LEVEL_HOUR, LEVEL_MINUTE, LEVEL_SECOND)
    ...
    val TYPE_FRIDAY = 5
    val TYPE_SATURDAY = 6
    val DAY_OF_WEEK_TYPES = intArrayOf(TYPE_SUNDAY, TYPE_MONDAY, TYPE_TUESDAY, TYPE_WEDNESDAY, TYPE_THURSDAY, TYPE_FRIDAY, TYPE_SATURDAY)
    ...
    val NAME_FRIDAY = "五"
    val NAME_SATURDAY = "六"
    val DAY_OF_WEEK_NAMES = arrayOf(NAME_SUNDAY, NAME_MONDAY, NAME_TUESDAY, NAME_WEDNESDAY, NAME_THURSDAY, NAME_FRIDAY, NAME_SATURDAY)
}
...
fun getDateDetail(time: Long): IntArray {
    ...
    return intArrayOf(mCalendar.get(Calendar.YEAR), //0
        ...
        if (nowDetails[1] == smartDetail[1]) {
            ...
            if (day >= 3) {
                ...
                if (0 == nowDetails[TimeUtils.HOUR_OF_DAY] - smartDetail[TimeUtils.HOUR_OF_DAY]) {
                    val minute = (nowDetails[TimeUtils.MINUTE] - smartDetail[TimeUtils.MINUTE]).toLong()
                    if (minute < 1) {
                        smartDate = "刚刚"
                    } else if (minute < 31) {
